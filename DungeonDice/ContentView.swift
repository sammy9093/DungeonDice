...
struct ContentView: View {
    ...
    enum Dice: Int, CaseIterable { //CaseIterable gets added to conform to iteration
        case four = 4
        case six = 6
        case eight = 8
        case ten = 10
    ...
    var body: some View {
        VStack {
            ...
            Group {
                ...
                // \.self needs to be unique or errors will occur. Learning a fix
                ForEach(Dice.allCases, id: \.self) { dice in
                    //Text("\(dice.rawValue)") // will error if doesn't follow case itterable protocol
